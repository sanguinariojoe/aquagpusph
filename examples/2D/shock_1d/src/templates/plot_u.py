...
def update(frame_index):
    ...
    try:
        ...
        return
    sph.set_data(x / L, u / CS)
    exp.set_data(exp_data['x'] / L, exp_data['u'] / CS)
    ax.set_title(r"$t \,\, c_0 / L = {}$".format(t / T))
    ax.set_ylim(0, np.max(np.concatenate([u, exp_data['u']])) / CS)
