...
// Initialize the output
	...
	BEGIN_NEIGHS(c_i, N, n_cells, icell, ihoc)
	{
		...
		const vec_xyz r_ij = r_i - r[j].XYZ;
		...
		}
		{
            ...
            float PI_ij = 0.0f;
            if (mu_ij < 0.0f){
                PI_ij = rho_ij_med_inv * (-alpha_mg * si_ij_med * mu_ij + beta_mg * mu_ij * mu_ij);
            }
            ...
            _GRADP_ -= m_j * (p_i/(rho_i*rho_i)+p_j/(rho_j*rho_j) + PI_ij) * r_ij * f_ij;

            _W_DEN_ +=  p[i] / (rho[i] * rho[i]) * local_div + 0.5f * m_j * PI_ij * v_dot_r * f_ij;
		}
	}
	END_NEIGHS()
