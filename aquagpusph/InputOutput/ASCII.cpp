	...
	for (i = 0; i < bounds().y - bounds().x; i++) {
		for (j = 0; j < fields.size(); j++) {
			...
			if (!type_name.compare("int*")) {
				...
				vec* v = (vec*)ptr;
#ifdef HAVE_3D
				f << std::setprecision(max_precision) << v[i].x << " " << v[i].y
				  << " " << v[i].z << " " << v[i].w << ",";
#else
				f << std::setprecision(max_precision) << v[i].x << " " << v[i].y
				  << ",";
				...
				matrix* m = (matrix*)ptr;
#ifdef HAVE_3D
				f << std::setprecision(max_precision) << m[i].s0 << " "
				  << m[i].s1 << " " << m[i].s2 << " " << m[i].s3 << " "
				  << m[i].s4 << " " << m[i].s5 << " " << m[i].s6 << " "
				  << m[i].s7 << " " << m[i].s8 << " " << m[i].s9 << " "
				  << m[i].sA << " " << m[i].sB << " " << m[i].sC << " "
				  << m[i].sD << " " << m[i].sE << " " << m[i].sF << ",";
#else
				f << std::setprecision(max_precision) << m[i].s0 << " "
				  << m[i].s1 << " " << m[i].s2 << " " << m[i].s3 << ",";
#endif // HAVE_3D
